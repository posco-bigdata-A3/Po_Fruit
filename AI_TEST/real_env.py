...
class RealEnv():
    ...
    def get_obs(self, save=False) -> EnvState:
        ...
        pred_scores = results[0]["scores"].detach().cpu().numpy() # 결과를 CPU로 이동
        pred_labels, pred_boxes = results[0]["labels"].detach().cpu().numpy(), results[0]["boxes"].detach().cpu().numpy() # 결과를 CPU로 이동
